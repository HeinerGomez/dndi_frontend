...
export class MyInfoComponent implements OnInit {
	heading = "My Info";
	subheading =
		'Complete the form, and click "Update my info" button afterwards.';
		...
		this.updateInfoForm = this.formBuilder.group(
			{
				...
				address: [
					"3515 NW 7th Ave, Miami, FL 33127, USA",
					[Validators.required],
				],
	...
	getCities = (state: any) => {
		...
		this.stateCities = this.cities.filter(
			(city: any) => city.id_state === state.id
		);
	};
}
